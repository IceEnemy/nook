<script>
    ...
    async function updateData(snapshot) {
        ...
        contactArr = await Promise.all(contacts.reverse().map(async contact => {
            const userDoc = await getDoc(doc(db, 'users', contact.uid));
            return { ...contact, username: userDoc.exists() ? userDoc.data().username : 'Unknown' };
        }));

        friendRequestArr = await Promise.all(friendRequests.reverse().map(async friendRequest => {
            const userDoc = await getDoc(doc(db, 'users', friendRequest));
            return { uid: friendRequest, username: userDoc.exists() ? userDoc.data().username : 'Unknown' };
        }));
    ...
    <div class="contactsList">
        ...
        <div class="msgs">
            {#each filteredContacts as contact}
                <Contact id={contact.chatId} uid={contact.uid} lastMessage={contact.lastMessage} username={contact.username} chosen={contact.chatId == chatId ? true : false} onClick={handleContactClick}/>
            {/each}

...
<div class="pageContainer">
    <form class="manageButtons">
        <div class="choiceButtons">
            <div class="dropdown" use:clickOutside = {'new'}>
                ...
                {#if dropdownOpen.new} <!-- Conditional rendering for dropdown content -->
            ...
            <div class="dropdown" use:clickOutside = {'filter'}>
                ...
                {#if dropdownOpen.filter} <!-- Conditional rendering for dropdown content -->
                    <div class="dropdown-content">
                        {#each filterOptions as option}
                            {#if option !== filterSelection}
                            <button on:click={() => {filterSelection = option; toggleDropdown('filter')}}>
                ...
                {#if dropdownOpen.sort}
                    <div class="dropdown-content">
                        {#each sortOptions as option}
                            ...
                            <button on:click={() => {sortSelection = option; toggleDropdown('sort')}}>
                                <p class="text">{option}</p>
                            </button>
                            {/if}
                        {/each}
        ...
        <div class="folderGrid">
            {#each filteredFolders as folder}
                <!-- {#if (searched != '' && folder.title.toLowerCase().includes(searched.toLowerCase())) || searched === ''} -->
                    <FolderPreview title={folder.title} link={folder.noteId} on:openPopup={listenEditDetail}/>
                <!-- {/if} -->
            {/each}
        </div>
    </div>
    ...
    {#if newPopup !== ''}
        <div class="changePopups" transition:fade={{ duration: 200 }}>
            <div class="changeInput" transition:scale={{ start:0.8, end:1,duration: 200}} use:clickOutside={'popup'}>
                {#if newPopup === 'folder'}
